<script lang="ts">
	...
	// Состояние формы для пользовательских параметров
	let newParamKey = $state('');
	let newParamValue = $state('');
	let isCreating = $state(false);
	let userParameters = $state<Record<string, string>>({});
  
	// Состояние для Vault-ключей
	let vaultKeys = $state<string[]>([]);
	let vaultLoading = $state(false);
	let vaultError = $state<string | null>(null);
	
	// Путь в Vault (можно вынести в настройки)
	const VAULT_PATH = '40serobabovas@region.cbr.ru';
  
	// Загрузка ключей из Vault при монтировании
	$effect(() => {
	  loadVaultKeys();
	});
	...
	const loadVaultKeys = async () => {
	  ...
	  try {
		// Кодируем путь для URL (особенно важно для @ и других спецсимволов)
	...
	// --- Пользовательские параметры (локальные) ---
	const startCreate = () => {
	  newParamKey = '';
	  newParamValue = '';
	  isCreating = true;
	...
	const handleDelete = (key: string) => {
	  const { [key]: _, ...rest } = userParameters;
	  userParameters = rest;
	...
	<div class="overflow-y-scroll max-h-[28rem] md:max-h-full space-y-4">
	  ...
	  <div class="space-y-2">
		<div class="flex items-center justify-between">
		  <div class="text-sm font-medium">{$i18n.t('Vault Keys')}</div>
		  <button
		...
		<p class="text-xs {$settings?.highContrastMode ? 'text-gray-800 dark:text-gray-100' : 'text-gray-500 dark:text-gray-400'}">
		  {$i18n.t('Keys from Vault path:')} <code class="bg-gray-100 dark:bg-gray-800 px-1 rounded">{VAULT_PATH}</code>
		</p>
		...
		{:else if vaultKeys.length > 0}
		  <div class="space-y-1">
			{#each vaultKeys as key}
			  <!-- Отображаем ТОЛЬКО ключ, как требовалось -->
		...
		{:else}
		  <div class="text-xs text-gray-400 italic">{$i18n.t('No keys found')}</div>
		{/if}
	  </div>
	  ...
	  <div class="space-y-2">
		...
		<p class="text-xs leading-relaxed {$settings?.highContrastMode ? 'text-gray-800 dark:text-gray-100' : 'text-gray-500 dark:text-gray-400'}">
		  {$i18n.t('User parameters placeholder hint')}
		</p>
  
		{#if Object.keys(userParameters).length > 0}
		  <div class="space-y-2 mt-2">
			{#each Object.entries(userParameters) as [key, value]}
			  <div class="flex items-center justify-between p-2 rounded-lg 
						  bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
				<!-- После сохранения показываем только ключ -->
				<span class="font-mono text-sm text-gray-800 dark:text-gray-200">{key}</span>
				<button
		...
		{#if isCreating}
		  <div class="mt-2 p-3 rounded-lg border border-blue-300 dark:border-blue-700 
