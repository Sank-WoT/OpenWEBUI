<script lang="ts">
	...
	import UserParameters from './Settings/UserParameters.svelte';
		...
		<div class="flex flex-col md:flex-row w-full pt-1 pb-4">
			...
			>
				...
				{#if filteredSettings.length > 0}
					{#each filteredSettings as tabId (tabId)}
						...
						{:else if tabId === 'user_parameters'}
							<button
								role="tab"
								aria-controls="tab-user_parameters"
								aria-selected={selectedTab === 'user_parameters'}
								class={`px-0.5 md:px-2.5 py-1 min-w-fit rounded-xl flex-1 md:flex-none flex text-left transition
								${
									selectedTab === 'user_parameters'
										? ($settings?.highContrastMode ?? false)
											? 'dark:bg-gray-800 bg-gray-200'
											: ''
										: ($settings?.highContrastMode ?? false)
							...
							>
								...
								<div class=" self-center">{$i18n.t('User Parameters')}</div>
							</button>
						{:else if tabId === 'audio'}
							<button
								role="tab"
			...
			<div class="flex-1 px-3.5 md:pl-0 md:pr-4.5 md:min-h-[42rem] max-h-[42rem]">
				...
				{:else if selectedTab === 'account'}
					...
					/>
				{:else if selectedTab === 'user_parameters'}
					<UserParameters {saveSettings} />
